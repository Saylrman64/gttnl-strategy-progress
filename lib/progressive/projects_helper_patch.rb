    ...
    base.class_eval do
      ...
      def render_version_cf_sorted_list(versions,cf_for_sorting)
        ...
        versions.each do |version|
          ...
          s << link_to(version.name,version_path(version)) + ' : '
          s << link_to_if(version.open_issues_count > 0,
            l(:label_x_open_issues_abbr, :count => version.open_issues_count),
            :controller => 'issues', :action => 'index', :project_id => project, :status_id => 'o', :fixed_version_id => version, :set_filter => 1)
          s << " / ".html_safe
          s << link_to_if(version.closed_issues_count > 0,
            l(:label_x_closed_issues_abbr, :count => version.closed_issues_count),
            :controller => 'issues', :action => 'index', :project_id => project, :status_id => 'c', :fixed_version_id => version, :set_filter => 1)
          if (version.effective_date && version.effective_date < Date.today)
              s << ',  <span class="red-text">' + due_date_distance_in_words(version.effective_date) + '</span>'
          end
          s << '<br/>'
          if User.current.member_of?(project)
            s << '<span class="version-project my-project">'
          else
            s << '<span class="version-project">'
          end
          s <<  l(:label_parent_project) + ' :  ' + '<span>'
          ...
          s << '<span class="version-project text-blue">' + ",  #{cf_for_sorting.name}: #{val}"  + '<span>' if val.present?
          s << '<div class="progressive-project-version margin-adjust">'
          s << "<br>" +  progress_bar([version.closed_percent, version.completed_percent], :width => '30em', :legend => ('%0.0f%' % version.completed_percent))
          s << "</div>"
          s << '</div></li>'
        end
        s << '</ul>'
      ...
      def render_project_progress_bars(project,options={})
        ...
        if project.versions.open.any?
          ...
          project.versions.open.reverse_each do |version|
            ...
            s << "<br/>" + l(:label_version) + " " + link_to_version(version) + ": " +
